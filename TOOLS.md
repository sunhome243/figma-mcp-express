...
**Agent presence (optional, 2.3.0+).** `origin` is an optional enum label (`grace`, `theo`, `sunho`, `zoe`, `taewon`, `emma`, `alex`, `rick`, `wolfgang`) the acting agent stamps on the outer `batch` call (and on read tools) so the plugin's multi-agent "Watch agent" panel can show who is working where (avatar + last action + status). An optional `status` enum (`thinking`/`waiting_review`/`reviewing`/`approved`/`escalated`/`done`) lets the orchestrator announce non-editing workflow transitions; most statuses are auto-derived from the op and cost nothing. Pass the SAME `origin` on every call from one agent. Both are display-only — not routing metadata — and harmlessly ignored when the presence panel is not in use. (A server predating 2.3.0 rejects unknown top-level params, so only send these to a 2.3.0+ server.) See `references/multi-agent.md § 7`.
...
| channel         | string   | No       | Target a specific connected file by channel id.                                                                                                                        |
| origin          | string   | No       | Presence label from the fixed roster (`grace`/`theo`/`sunho`/`zoe`/`taewon`/`emma`/`alex`/`rick`/`wolfgang`). Attributes this write to a named agent in the plugin's Watch-agent panel; ignored otherwise. Pass the same value on every call from one agent. (2.3.0+) |
