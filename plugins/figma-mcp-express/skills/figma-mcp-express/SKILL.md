...
6. For a remote follower, read `references/remote-follower.md`. The AI client connects to the follower's local `/mcp`, never the leader `.ts.net` URL; ask the user to set up Tailnet services rather than attempting it itself.
...
| Private Tailnet leader/follower setup and remote readiness | `references/remote-follower.md` |
