...
func registerReadExportTools(s *server.MCPServer, node *Node) {
	...
	s.AddTool(mcp.NewTool("export_frames_to_pdf",
		...
		return executeExportFramesToPDF(ctx, node, req, nodeIDs, outputPath)
	})
...
func executeExportFramesToPDF(ctx context.Context, node *Node, req mcp.CallToolRequest, nodeIDs []string, outputPath string) (*mcp.CallToolResult, error) {
	...
	resp, err := node.Send(ctx, "export_frames_to_pdf", nodeIDs, withChannel(req, nil))
	if err != nil {
		return mcp.NewToolResultError(err.Error()), nil
	}
	if resp.Error != "" {
