...
func registerWriteStyleTools(s *server.MCPServer, node *Node) {
	...
	s.AddTool(mcp.NewTool("set_effects",
		...
		mcp.WithArray("effects",
			mcp.Required(),
			mcp.Description("Array of effect objects. type: DROP_SHADOW | INNER_SHADOW | LAYER_BLUR | BACKGROUND_BLUR | GLASS | NOISE | TEXTURE. Shadow fields: color, opacity, offsetX/Y, radius, spread, showShadowBehindNode. GLASS/NOISE/TEXTURE take their native Figma params (all defaulted)."),
			mcp.Items(map[string]any{"type": "object"}),
		),
		channelParam(),
	), func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
