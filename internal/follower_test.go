...
// ── followerClientTimeout ─────────────────────────────────────────────────────

// TestFollowerClientTimeout_Default verifies that with no env var set the
// timeout is at least 125s (floor 120s + 5s headroom).
func TestFollowerClientTimeout_Default(t *testing.T) {
	t.Setenv("FIGMA_MCP_TIMEOUT", "")
	got := followerClientTimeout()
	if got < 125*time.Second {
		t.Errorf("default followerClientTimeout = %v, want >= 125s", got)
	}
}

// TestFollowerClientTimeout_CustomLarge verifies that FIGMA_MCP_TIMEOUT=180
// yields at least 185s (180 + 5s headroom).
func TestFollowerClientTimeout_CustomLarge(t *testing.T) {
	t.Setenv("FIGMA_MCP_TIMEOUT", "180")
	got := followerClientTimeout()
	if got < 185*time.Second {
		t.Errorf("followerClientTimeout with 180s = %v, want >= 185s", got)
	}
}

// TestFollowerClientTimeout_NeverBelow125 verifies that very small env values
// are floored so the result is never below 125s.
func TestFollowerClientTimeout_NeverBelow125(t *testing.T) {
	t.Setenv("FIGMA_MCP_TIMEOUT", "1")
	got := followerClientTimeout()
	if got < 125*time.Second {
		t.Errorf("followerClientTimeout with small value = %v, want >= 125s", got)
	}
}
